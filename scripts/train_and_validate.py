...
def degradation_two_way_cv(laps: pd.DataFrame, stints: pd.DataFrame) -> dict:
    ...
    n_pred = sum(
        (row.Compound, row.CircuitName) in model.curves or row.Compound in model.global_curves
        ...
        rows.append(
            {
                ...
                "expected_fresh_pace_delta": -float(
                    deg_model.predict_delta(compound, circuit, tyre_age)[0]
                )
...
def train_undercut(all_laps: pd.DataFrame, pits: pd.DataFrame, deg_model: DegradationModel) -> dict:
    ...
    if len(feats) < 30:
        return {
            "error": f"only {len(feats)} examples - need more races for a real model",
            "n": len(feats),
        }
...
def run_simulator(deg_model: DegradationModel) -> dict:
    config = RaceConfig(
        circuit="Italian Grand Prix", total_laps=53, sc_rate_per_race=1.0, overtake_difficulty_s=0.3
    )
...
def main() -> None:
    ...
    print("  Within-circuit holdout (random 20% of stints):")
    print(f"    test rows:       {deg['n_test_within']:,}")
    print(f"    MAE:             {deg['within_circuit_mae_s']:.3f} s")
    print("  Cross-circuit holdout (Italian GP held out):")
    print(f"    test rows:       {deg['n_test_cross']:,}")
    print(f"    MAE:             {deg['cross_circuit_mae_s']:.3f} s")
    print(
        f"  Production model: {deg['prod_n_curves']} per-circuit curves, {deg['prod_n_global']} fallback"
    )
    ...
    for _, r in cost.iterrows():
        print(
            f"  {r['CircuitName']:30s}  median={r['MedianPitLossS']:5.2f}s  "
    ...
    if "error" in uc:
        ...
        print(
            f"  examples:        {uc['n_total']} ({int(uc['base_rate'] * uc['n_total'])} positive)"
        )
    ...
    print(
        f"  B (2-stop, grid P3):       E[finish]={s['B_expected_finish']:.2f}  dist={s['B_dist']}"
    )
